...
async def render_weather(request: Request, city_name: str, view_type: str) -> HTMLResponse:
    # If the view type is 'current', fetch current weather data and render the current.html template
    if view_type == 'current':
        current: CurrentWeather = await get_current(city_name)
    ...
    if view_type == 'forecast':
        forecast: ForecastWeather = await get_forecast(city_name)
